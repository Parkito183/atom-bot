#!/usr/bin/env python3
"""
backtest_v4.py — BTC como Filtro de Confirmación
ADA da la señal, BTC valida la dirección.
"""
...
ESTRATEGIA = {
    'nombre': 'BTC Confirmation Filter',
    'apalancamiento': 3,
    'comision': 0.0005,
    
    # Filtro BTC: qué tan alineado debe estar
    'btc_confirmacion': True,      # True = requiere BTC en misma dirección
    'btc_emergencia': True,        # True = salir si BTC cambia en contra
...
def detectar_regimen_btc(velas, i):
    """Detecta régimen de BTC usando EMA50 + Supertrend."""
    v = velas[i]
    precio = v['close']
    ema50 = v.get('ema50')
    st = v.get('supertrend')
...
def buscar_entrada(ada_velas, btc_velas, i, c):
    ...
    # === LONG: Requiere BTC alcista o neutral ===
    if st_prev == 'bajista' and st_now == 'alcista':
        if c['rsi_long_min'] <= rsi <= c['rsi_long_max']:
            # FILTRO BTC: Si BTC está bajista, no entrar LONG
    ...
    # === SHORT: Requiere BTC bajista o neutral ===
    if st_prev == 'alcista' and st_now == 'bajista':
        if c['rsi_short_min'] <= rsi <= c['rsi_short_max']:
            # FILTRO BTC: Si BTC está alcista, no entrar SHORT
...
def evaluar_salida(trade, ada_velas, btc_velas, i, c):
    ...
    # EMERGENCIA BTC: Si BTC cambia drásticamente en contra
...
def imprimir_reporte(trades):
    print("\n" + "="*70)
    print(f"📊 BACKTEST V4 — {ESTRATEGIA['nombre']}")
    print("="*70)
